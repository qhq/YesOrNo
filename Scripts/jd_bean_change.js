...
[task_local]
#京东资产变动通知
2 8 * * * https://gitee.com/lxk0301/jd_scripts/raw/master/jd_bean_change.js, tag=京东资产变动通知, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true
================Loon===============
[Script]
cron "2 8 * * *" script-path=https://gitee.com/lxk0301/jd_scripts/raw/master/jd_bean_change.js, tag=京东资产变动通知
=============Surge===========
[Script]
京东资产变动通知 = type=cron,cronexp="2 8 * * *",wake-system=1,timeout=3600,script-path=https://gitee.com/lxk0301/jd_scripts/raw/master/jd_bean_change.js
...
async function showMsg() {
  if ($.errorMsg) return
  allMessage += `账号${$.index}：${$.UserName}\n今日收入：${$.todayIncomeBean}京豆\n昨日收入：${$.incomeBean}京豆\n昨日支出：${$.expenseBean}京豆\n当前京豆：${$.beanCount}(今过期${$.expirejingdou})京豆${$.message}${$.index !== cookiesArr.length ? '\n\n' : ''}`;
  if ($.isNode() && ONE_BY_ONE == 'true') {
    await notify.sendNotify2(`${$.name} - 账号${$.index} - ${$.UserName}`, `账号${$.index}：${$.UserName}\n今日收入：${$.todayIncomeBean}京豆\n昨日收入：${$.incomeBean}京豆\n昨日支出：${$.expenseBean}京豆\n当前京豆：${$.beanCount}(今过期${$.expirejingdou})京豆${$.message}`, { url: `https://bean.m.jd.com/beanDetail/index.action?resourceValue=bean` })
  }
  $.msg($.name, '', `账号${$.index}：${$.UserName}\n今日收入：${$.todayIncomeBean}京豆\n昨日收入：${$.incomeBean}京豆\n昨日支出：${$.expenseBean}京豆\n当前京豆：${$.beanCount}(今过期${$.expirejingdou})京豆${$.message}`, {"open-url": "https://bean.m.jd.com/beanDetail/index.action?resourceValue=bean"});
}
async function bean() {
        ...
        if (err) {
          ...
          if (data) {
            ...
            if (data['retcode'] === "0" && data.data && data.data.hasOwnProperty("userInfo")) {
              ...
              //$.custName = (process.env.CUSTNAME ? process.env.CUSTNAME : '').split(',')[i];
            }
