...
!(async () => {
    ...
    let result = JSON.parse(await refreshKey(jsStr))

    if (result.ok == 0) {
        await notify.sendNotify(`${$.name}`, result.msg, ``);
    } else {
    ...
    return new Promise(resolve => {
        $.get(getOptions(indexUrl), (err, resp, data) => {
            if (err) {
                ...
                const content = fs.readFileSync("../config/config.sh", "utf8");
                if (invokeKey) {
                    if (content.indexOf(invokeKey) == -1) {
                        try {
                            data = content.replace(/export invokeKey=".*?"/g, `export invokeKey="${invokeKey}"`)
                        } catch (e) { }
                        //fs.writeFileSync( './execute.js', content, 'utf8')
                        fs.writeFile('../config/config.sh', data, function (err, data) {
                            if (err) {
                                resolve(`{"ok":-1,"msg":"刷新invokeKey失败"}`)
                                throw err;
                            } else {
                                resolve(`{"ok":0,"msg":"刷新${invokeKey}失败"}`)
                            }
                        })
                    }else {
                        resolve(`{"ok":-1,"msg":"invokeKey无变化：${invokeKey}"}`)
                    }
                } else {
                    resolve(`{"ok":-1,"msg":"刷新invokeKey失败"}`)
                }
