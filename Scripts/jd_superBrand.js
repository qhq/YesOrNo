...
[task_local]
#特物Z|万物皆可国创
2 0,2 * * * https://raw.githubusercontent.com/Wenmoux/scripts/master/jd/jd_superBrand.js, tag=特物Z|万物皆可国创, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true
...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        if (cookie) {
            ...
            console.log(`\n******开始【京东账号${$.index}】${$.custName || $.custName || $.nickName || $.UserName}*********\n`);
            if (!$.isLogin) {
                $.msg($.name, `【提示】cookie已失效`, `京东账号${$.index} ${$.custName || $.custName || $.nickName || $.UserName}\n请重新登录获取\nhttps://bean.m.jd.com/bean/signIndex.action`, {
            ...
            if ($.cando) {
                ...
                //console.log($.taskList[2].ext)
                await doTask("secondfloor", $.enpid, $.taskList[2].encryptAssignmentId, $.taskList[2].ext.assistTaskDetail.itemId, $.taskList[2].assignmentType)
            ...
            try {
                if (err) {
                    ...
                    if (data && data.data && data.code === "0" && source === "secondfloor") {
                        if (data.data.result && data.data.result.taskList && data.data.result.taskList[3] ? data.data.result.taskList[3] : data.data.result.taskList[2]) {
                            $.taskList = data.data.result.taskList
                            let result = data.data.result.taskList[3] ? data.data.result.taskList[3] : data.data.result.taskList[2]
