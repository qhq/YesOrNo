    ...
    if ($request.headers && $request.url.indexOf('GetJDUserInfoUnion') > -1) {
      ...
      if (CV.match(/(pt_key=.+?pt_pin=|pt_pin=.+?pt_key=)/)) {
        ...
        if ($ql.ql) {
          ...
          msg = (await $ql.addEnvs(CookieValue)).msg;
          //console.log(msg);

          //$.notify('用户名: ' + msg, '', '同步更新容器成功🎉');
...
function QL_API() {
  return new (class QL {
    constructor() {
      ...
      const ipAddress = this.$.read('#qhq_HOSTURL') || '';
    ...
    addEnvs(cookies) {
      ...
      this.headers['api-token'] = this.$.read('#qhq_APITOKEN') || '';
      const opt = {
        url: this.getURL('updateCookie'),
        headers: this.headers,
