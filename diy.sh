...
sed -i "/旧的可继续使用/a await $.get({url: 'http://51.15.187.136:8080/activeJdCfdCode?code=' + strMyShareId}, function (err, resp, data) {console.log('互助码状态:' + resp.body);})" $${ScriptsDir}/jd_cfd.js >/dev/null 2>&1 && echo -e " 财富岛passerby互助码激活已添加"
...
## 注释指定活动
echo -e "+---------------- 暂时停用脚本 ---------------+"
js_List="longzhuzhu_long_super_redrain longzhuzhu_long_half_redrain jd_joy_run jd_joy_feedPets jd_bean_change jd_carnivalcity passerby_jd_fruit2 passerby_jd_dreamFactory2 passerby_jd_cfd2"
