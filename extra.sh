...
#[ $(grep -c "NOTIFY_SKIP_LIST" ${ScriptsDir}/function/sendNotify.js) -eq 0 ] && sed -i "/Air/a\    const notifySkipList = process.env.NOTIFY_SKIP_LIST ? process.env.NOTIFY_SKIP_LIST.split('&') : [];\n    const titleIndex = notifySkipList.findIndex((item) => item === text);\n    if (titleIndex !== -1) {\n      console.log(\`\\n🔕🔕 ${text} 在推送黑名单中，已跳过推送 🔕🔕\\n\`);\n      return;\n    }" ${ScriptsDir}/function/sendNotify.js >/dev/null 2>&1 && echo -e " 通知黑名单已添加"
...
if [[ $iCan = "true" ]]; then
    if [ $(grep -c "open_access_token" ${ScriptsDir}/Sunert_txnews.js) -eq 0 ]; then
        echo -e " 腾讯新闻"
    ...
    if [ $(grep -c "WISE_HIS_PM" ${ScriptsDir}/Sunert_baidu_speed.js) -eq 0 ]; then
        echo -e " 百度极速"
        sed -i "60r ${ScriptsDir}/Sunert_baidu_speed.txt" ${ScriptsDir}/Sunert_baidu_speed.js
    fi
    if [ $(grep -c "kd.youth.cn" ${ScriptsDir}/Sunert_youth.js) -eq 0 ]; then
        echo -e " 中青看点"
...
wget -q ${ProxyJudge}https://raw.githubusercontent.com/JDHelloWorld/jd_scripts/main/TS_USER_AGENTS.ts -O ${ScriptsDir}/TS_USER_AGENTS.ts
...
perl -0777 -i -pe 's|https:\/\/raw\.githubusercontent\.com\/qhq\/YesOrNo\/main\/extra\.sh|http://49.235.99.245:6300/qhq/YesOrNo/raw/branch/main/extra.sh|i' ${ConfigDir}/config.sh >/dev/null 2>&1
...
sed -i '/sendNotify 推送通知功能/a const diy = true;' $ScriptsDir/sendNotify.js
...
if [ -n "$js_List" ]; then
    echo -e "+----------------- 停用脚本 -----------------+"
    for js_item in $js_List; do
        sed -i "s|\(^[0-9].*task $js_item[$| ]\)|# \1|" $ListCrontabUser && echo -e " \033[32m[已停用]\033[0m $js_item"
...
## 强制取消定时注释
js_List="qhqcz_jd_ckcheck qhqcz_post_code"
if [ -n "$js_List" ]; then
    echo -e "+----------------- 开启脚本 -----------------+"
    for js_item in $js_List; do
        #sed -i "s/^#\([0-9].*task $js_item[$| ]\)/\1/g" $ListCrontabUser
        sed -i "s/^#[ |]\([0-9].*task $js_item[$| ]\)/\1/g" $ListCrontabUser && echo -e " \033[32m[已开启]\033[0m $js_item"
        #[ $(grep -c "^[0-9].*task $js_item" $ListCrontabUser) -ne 0 ] && echo -e " \033[32m[已开启]\033[0m $js_item"
        #sed -i "/$js_item/d" $ListCrontabUser && echo -e "$js_item已删除"
    done
    echo -e "+--------------------------------------------+\n"
fi
...
## 删除过期活动
js_List="airacg_jd-reward-joy airacg_jd-task-price airacg_jd-task-invokeKey airacg_jd-task-validate airacg_jx-help-cashback JDHelloWorld_jd_api_test Tsukasa007_jd_joypark_joy Tsukasa007_jd_joypark_task moposmall_jx_mc_coin moposmall_jx_mc_emptycabbage shufflewzc_jd_try Wenmoux_jd_ddnc_farmpark"
if [ -n "$js_List" ]; then
    echo -e "+----------------- 删除脚本 -----------------+"
    for js_item in $js_List; do
        [ -e ${ScriptsDir}/$js_item.js ] && rm -rf ${ScriptsDir}/$js_item.js && sed -i "/task $js_item[$| ]/d" $ListCrontabUser && echo -e " \033[32m[已删除]\033[0m $js_item.js"
        [ -e ${ScriptsDir}/$js_item.ts ] && rm -rf ${ScriptsDir}/$js_item.ts && sed -i "/task $js_item[$| ]/d" $ListCrontabUser && echo -e " \033[32m[已删除]\033[0m $js_item.ts"
        [ -e ${ScriptsDir}/$js_item.py ] && rm -rf ${ScriptsDir}/$js_item.py && sed -i "/task $js_item[$| ]/d" $ListCrontabUser && echo -e " \033[32m[已删除]\033[0m $js_item.py"
    done
    echo -e "+--------------------------------------------+\n"
fi
